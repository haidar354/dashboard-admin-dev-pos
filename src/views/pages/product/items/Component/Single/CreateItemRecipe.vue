...
<template>
  ...
  >
    ...
    <VCardText>
      <VRow>
        <VCol
          v-if="createForm.useSameConfig && createForm.config?.saleFulfillmentMode !== 'RECIPE'"
          cols="12"
        >
          <VAlert
            type="warning"
        ...
        >
          <VRow>
            ...
            >
              <div
                v-if="createForm.hasVariant && createForm.config?.saleFulfillmentMode === 'RECIPE'"
                class="mb-4"
              >
                <VAlert
                  type="info"
                ...
                >
                  <VAlertTitle>Informasi Resep</VAlertTitle>
                  <div>
                    Terdapat <span>{{ createForm.skus?.length }}</span> SKU untuk produk ini.
                  </div>
                </VAlert>
                <div>
              ...
              <template v-if="createForm.config?.recipePolicy === 'ITEM_BOM_ONLY'">
                <template v-if="(createForm.bom?.lines?.length || 0) > 0">
                  <VRow
                    v-for="(recipe, index) in (createForm.bom?.lines || [])"
                    :key="index"
                    class="mb-4"
                  >
                    <VCol cols="12">
              ...
              <template v-else-if="createForm.config?.recipePolicy === 'SKU_BOM_ONLY'">
                <VAlert type="warning">
