...
<template>
  ...
  >
    ...
    <VCardText>
      <div v-if="updateForm.units && (updateForm.units?.length || 0) > 0">
        <VRow
          v-for="(unitLine, index) in updateForm.units"
          :key="index"
          class="mb-4"
        >
          <VCol cols="12">
        ...
        <VBtn
          ...
          block
          :disabled="(updateForm.units.length || 0) >= 5 || !!updateForm.units?.find(line => !line.unit)"
          @click="addUnit"
        >
          Tambah Satuan Lain
        </VBtn>
        ...
        <!--
          ...
          <VSelect
          v-model="updateForm.itemUnit.stockUnitId"
          :rules="[requiredValidator]"
          :items="updateForm.itemUnit.lines.map((line, idx) => ({ title: line.unit?.name || `Satuan ${idx + 1}`, value: line.unitId }))"
          :min="1"
          variant="outlined"
          ...
          <VSelect
          v-model="updateForm.itemUnit.purchaseUnitId"
          :rules="[requiredValidator]"
          :items="updateForm.itemUnit.lines.map((line, idx) => ({ title: line.unit?.name || `Satuan ${idx + 1}`, value: line.unitId }))"
          :min="1"
          variant="outlined"
          ...
          <VSelect
          v-model="updateForm.itemUnit.salesUnitId"
          :rules="[requiredValidator]"
          :items="updateForm.itemUnit.lines.map((line, idx) => ({ title: line.unit?.name || `Satuan ${idx + 1}`, value: line.unitId }))"
          :min="1"
          variant="outlined"
          ...
          <VSelect
          v-model="updateForm.itemUnit.transferUnitId"
          :rules="[requiredValidator]"
          :items="updateForm.itemUnit.lines.map((line, idx) => ({ title: line.unit?.name || `Satuan ${idx + 1}`, value: line.unitId }))"
          :min="1"
          variant="outlined"
