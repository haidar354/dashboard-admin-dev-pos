...
const headers: ReadonlyHeaders = [
  ...
  { title: 'Nama Outlet', key: 'name', align: 'start', sortable: true },
  { title: 'Nama Unit Bisnis', key: 'businessUnit.name', align: 'start', width: '15%', sortable: true },
  { title: 'Provinsi', key: 'province.name', align: 'start', width: '15%', sortable: true },
  { title: 'Kota', key: 'city.name', align: 'start', width: '15%', sortable: true },
  { title: 'Tipe', key: 'isCentral', align: 'start', width: '15%', sortable: false },
...
const fetchPaginatedData = async () => {
  await outletStore.fetchPaginate({
    include: ['province', 'city', 'businessUnit'],
  })
}
    ...
    <VCard>
      <VCardText class="d-flex flex-wrap justify-space-between">
        ...
        <div class="mt-3 mt-md-0 flex-fill">
          <div class="w-auto d-flex flex-wrap justify-end">
            ...
            <VBtn
              v-if="$can('manage', 'Tambah Outlet') || $can('manage', 'Tambah Semua Outlet') || $can('manage', 'default')"
              class="ms-2"
              color="primary"
              :to="{
                name: 'setting-outlets-create',
              }"
            >
              <VIcon
                start
      ...
      <VExpandTransition>
        ...
        >
          ...
          <VCardText>
            <VForm @submit.prevent="outletStore.applyFilter()">
              <VRow>
                <VCol
                  cols="12"
      ...
      <VDataTable
        ...
        @update:sort-by="outletStore.onSortBy"
      ...
      >
        ...
        <template #item.businessUnit.name="{ item }">
          <div class="text-wrap">
        ...
        <template #item.isCentral="{ item }">
          <VChip
            :color="item.isCentral ? 'success' : 'info'"
            :variant="item.isCentral ? 'flat' : 'outlined'"
        ...
        <template #item.Aksi="{ item }">
          <div class="d-flex align-center justify-center">
            <IconBtn
              :disabled="!($can('manage', 'Lihat Outlet') || $can('manage', 'default'))"
              :to="{ name: 'setting-outlets-outlet-id', params: { outletId: item.outletId } }"
            >
              <VIcon
                color="primary"
                icon="tabler-eye"
