...
async function handleSubmit() {
  try {
    ...
    const updatedMenu = await menuStore.update(menuId)
    if (updatedMenu) {
      await router.push({
...
function handleCancel() {
  ...
  router.push({ name: 'sales-menus-menu-id', params: { menuId } })
}
...
onMounted(async () => {
  ...
  // Set form data after fetching menu detail
  if (menuStore.selectedMenu.menuId) {
    menuStore.setFormData(menuStore.selectedMenu)

    // Determine mode based on existing outlets
    if (menuStore.form.outlets.length > 1) {
      isMultiOutlet.value = true
      selectedOutlets.value = menuStore.form.outlets.map(o => o.outletId)

      // Set global pricing from first outlet
      const firstOutlet = menuStore.form.outlets[0]

      globalPricing.value = {
        priceDineIn: firstOutlet.priceDineIn,
        priceTakeAway: firstOutlet.priceTakeAway,
        priceOnlineOrder: firstOutlet.priceOnlineOrder,
      }
    }
    else if (menuStore.form.outlets.length === 1) {
      isMultiOutlet.value = false

      const outlet = menuStore.form.outlets[0]

      singleOutletPricing.value = {
        priceDineIn: outlet.priceDineIn,
        priceTakeAway: outlet.priceTakeAway,
        priceOnlineOrder: outlet.priceOnlineOrder,
        menuCategoryId: outlet.menuCategoryId,
      }
    }
    else {
      // No outlets configured, default to single outlet with sidebar outlet
      isMultiOutlet.value = false
      if (selectedSidebarOutlet.value?.outletId)
        syncFormWithMode()
    }
        ...
        <div class="d-flex align-center justify-space-between">
          <div>
            ...
            <VCardSubtitle class="text-body-1">
              Perbarui informasi untuk: {{ menuStore.selectedMenu.name }}
            </VCardSubtitle>
          </div>
          <VBtn
            color="secondary"
        ...
        <VForm @submit.prevent="handleSubmit">
          ...
          <div class="d-flex justify-end gap-4">
            ...
            <VBtn
              ...
              :loading="isLoading || menuStore.isLoadingUpdate"
