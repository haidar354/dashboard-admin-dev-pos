...
// Form Data
const form = ref({
  invoiceNumber: '', // Optional manual input
...
watch(invoiceMode, newMode => {
  if (newMode === 'standalone') {
    ...
    // Load initial SO list when switching to from_order mode
    loadInitialSalesOrders()
  }
})

// Watch SO selection
watch(selectedSalesOrderId, async soId => {
  if (soId && invoiceMode.value === 'from_order')
    await loadSalesOrderData(soId)
})
  ...
  ([terms, date]) => {
    ...
    switch (terms) {
      ...
      case 'COD':
        ...
        break
      default:
        return // Don't change if custom or unknown
    }
...
const fetchSalesOrders = async (query?: string) => {
  // Allow initial load without query, or search with min 2 chars
  if (query && query.length < 2)
    return

  isLoadingSalesOrders.value = true
  try {
    // Use salesOrderStore to fetch
    ...
    if (response.data) {
      console.log('SO Response:', response.data) // Debug
...
const loadInitialSalesOrders = async () => {
  await fetchSalesOrders() // Load without query to get first 10
}
...
const loadSalesOrderData = async (salesOrderId: string) => {
  ...
  try {
    await salesOrderStore.fetchDetail(salesOrderId, {
      include: 'order,order.lines,order.lines.itemSku',
    })

    const so = salesOrderStore.selectedSalesOrder
    if (so && so.order) {
      // Fill customer info
      form.value.customerName = so.order.customerName || ''
      form.value.customerPhone = so.order.customerPhone || ''
      form.value.customerId = so.order.customerId || null

      // Fill lines from SO with maxQuantity for validation
      form.value.lines = (so.order.lines || []).map((line: any) => ({
        orderLineId: line.orderLineId,
        itemSkuId: line.itemSkuId,
        itemName: line.itemName,
        itemSku: line.code,
        quantity: line.quantity,
        maxQuantity: line.quantity, // Store original qty for validation
    ...
    showToast('Gagal memuat data Sales Order', 'error')
    console.error(error)
  }
  finally {
...
const fetchCustomers = async (query: string) => {
  // Implement customer search if needed
...
const onItemSelected = (item: any) => {
  ...
  form.value.lines.push({
    orderLineId: null, // Standalone item
...
const handleSubmit = async (isDraft: boolean = false) => {
  // Validation
  if (invoiceMode.value === 'from_order' && !selectedSalesOrderId.value) {
    showToast('Pilih Sales Order terlebih dahulu', 'error')
  ...
  // Validate quantities for from_order mode
  if (invoiceMode.value === 'from_order') {
    for (const line of form.value.lines) {
      if (line.maxQuantity && line.quantity > line.maxQuantity) {
        showToast(
          `Quantity untuk ${line.itemName} melebihi quantity order (max: ${line.maxQuantity})`,
          'error',
        )
  ...
  try {
    ...
    // Add SO reference if from order
    if (invoiceMode.value === 'from_order') {
      payload.salesOrderId = selectedSalesOrderId.value
    }
    else {
      // Standalone - add customer info
    ...
    const response: any = await $salesAPI('/invoices', {
      method: 'POST',
      body: payload,
    })
    ...
    showToast(error.data?.message || 'Gagal membuat invoice', 'error')
  }
  finally {
...
watch(
  salesOrderSearch,
  customDebounce(val => {
    // Only fetch if value is not already in options (prevent fetch on selection)
...
onMounted(() => {
  // Check query params for SO
  if (route.query.salesOrderId) {
    invoiceMode.value = 'from_order'
    selectedSalesOrderId.value = route.query.salesOrderId as string
  }
  else if (invoiceMode.value === 'from_order') {
    // Load initial SO list if already in from_order mode
    loadInitialSalesOrders()
  }
})
      ...
      >
        ...
        >
          <VCardText>
            ...
            >
              <VRadio
                label="Tanpa Referensi"
                value="standalone"
              />
              <VRadio
                label="Dari Penjualan (SO)"
                value="from_order"
              />
            </VRadioGroup>
          </VCardText>
        ...
        <!-- Sales Order Selection (if from_order) -->
        <VCard
          v-if="invoiceMode === 'from_order'"
          title="Pilih Pesanan Penjualan (SO)"
          class="mb-4"
        >
          <VCardText>
            <VAutocomplete
              ...
              label="Cari Pesanan Penjualan (SO)"
              placeholder="Ketik nomor SO..."
              :loading="isLoadingSalesOrders"
              :rules="[
                (value) => !!value || 'Pesanan Penjualan (SO) harus dipilih',
              ]"
              clearable
            >
              <template #item="{ props, item }">
                <VListItem v-bind="props">
                  <template #subtitle>
                    ...
                    {{ formatCurrency(item.raw.grandTotal || 0) }}
                  </template>
                </VListItem>
              </template>
            </VAutocomplete>
            ...
            <VTable class="text-no-wrap">
              ...
              <tbody>
                ...
                <tr v-if="form.lines.length === 0">
                  ...
                  >
                    <template v-if="invoiceMode === 'from_order'">
                      Pilih Sales Order untuk memuat items
                    </template>
                    <template v-else>
