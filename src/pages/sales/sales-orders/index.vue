...
const onRowClick = (item: SalesOrder, event: MouseEvent | PointerEvent) => {
...
watch(
  ...
  customDebounce(() => {
    salesOrderStore.fetchPaginatedData()
  }, 500),
)
  ...
  <div>
    <VCard>
      <VCardText class="d-flex flex-wrap justify-space-between">
        <span class="text-h5 d-inline my-auto">Pesanan Penjualan (SO)</span>
        <div class="mt-3 mt-md-0 flex-fill">
          <div class="w-auto d-flex flex-wrap justify-end">
            <VBtn
              color="primary"
              prepend-icon="tabler-plus"
              @click="$router.push('/sales/sales-orders/create')"
            >
      ...
      >
        <template #item="{ index, item }">
          ...
          >
            ...
            <td>
              {{ dayjs(item.orderDate).format("DD-MM-YYYY HH:mm") }}
            </td>
            <td class="text-center">
              <VBtn
        ...
        <template #no-data>
          ...
          >
            ...
            <div>Data Pesanan Penjualan (SO) tidak tersedia</div>
          </VAlert>
        </template>
      ...
      >
        ...
        >
          <VListItem
            ...
            @click="
              $router.push(`/sales/sales-orders/${selectedOrder.salesOrderId}`)
            "
          />
          <VListItem
            title="Download PDF"
            prepend-icon="tabler-file-download"
            :loading="salesOrderStore.isLoadingDownload"
            @click="salesOrderStore.downloadPdf(selectedOrder)"
          />
          <VListItem
            ...
            @click="
              $router.push(
                `/sales/sales-orders/${selectedOrder.salesOrderId}/edit`,
              )
            "
          />
          <VDivider class="my-1" />
