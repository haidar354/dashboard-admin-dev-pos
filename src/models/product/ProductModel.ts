...
// Pastikan $productAPI.axios kompatibel dengan Axios (data, params, headers)
export default class ProductModel<T = any> extends BaseModel {
  declare data: T
  ...
  async request(config: HTTPRequestConfig) {
    ...
    return await $productAPI.axios(config.url!, {
